...
//					createMyHeadFolder(userNum);
                    // 创建缓存文件夹
                    createRegisterFolder(userNum);
                }
    ...
    private static boolean createUserFolder(String userNum) {
        try {
            File dirFirstFile = new File(AppDisk.appInursePath + userNum
                    + File.separator);// 新建一级主目录
            if (!dirFirstFile.exists()) {// 判断文件夹目录是否存在
                dirFirstFile.mkdir();// 如果不存在则创建
            }
        } catch (Exception e) {
    ...
    private static void createRecordFolder(String userNum) {
        File dirFirstFile = new File(AppDisk.appInursePath + userNum
                + File.separator + AppDisk.DCIM_RECORD);// 新建一级主目录
        if (!dirFirstFile.exists()) {// 判断文件夹目录是否存在
            dirFirstFile.mkdir();// 如果不存在则创建
        }
    }
    ...
    private static void createVideoFolder(String userNum) {
        File dirFirstFile = new File(AppDisk.appInursePath + userNum
                + File.separator + AppDisk.DCIM_VIDEO);// 新建一级主目录
        if (!dirFirstFile.exists()) {// 判断文件夹目录是否存在
            dirFirstFile.mkdir();// 如果不存在则创建
        }
    }
    ...
    // //应用根目录创建成功之后再创建下级目录
    // //用户目录创建成功则创建用户下面的目录
    // File dirFirstFile=new
    // File(AppDisk.appInursePath+AppDisk.PIC_CACHE);//新建一级主目录
    // if(!dirFirstFile.exists()){//判断文件夹目录是否存在
    // dirFirstFile.mkdir();//如果不存在则创建
    // }
    // }
     ...
     * 创建缓存照片目录
     */
    public static void createRegisterFolder(String userNum) {
        ...
        if (hasSDCard()) {
            ...
            if (createRootFolder()) {
                ...
                if (createUserFolder(userNum)) {
                    // 用户目录创建成功则创建用户下面的目录
                    File dirFirstFile = new File(AppDisk.appInursePath
                            + userNum + File.separator + AppDisk.CACHE);// 新建一级主目录
                    if (!dirFirstFile.exists()) {// 判断文件夹目录是否存在
                        dirFirstFile.mkdir();// 如果不存在则创建
                    }
                    AppDisk.PICTUREPOLDER = dirFirstFile.getAbsolutePath()
                            + File.separator;
                }
...
//	 * 创建头像目录
//	 */
//	private static void createMyHeadFolder(String userNum) {
//		File dirFirstFile = new File(AppDisk.appInursePath + userNum
//				+ File.separator + AppDisk.MYHEAD);// 新建头像目录
//		if (!dirFirstFile.exists()) {// 判断文件夹目录是否存在
//			dirFirstFile.mkdir();// 如果不存在则创建
//		}
//	}
