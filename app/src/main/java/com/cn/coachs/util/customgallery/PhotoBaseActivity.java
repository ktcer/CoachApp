...
import com.cn.coachs.util.BitmapUtils;
import com.cn.coachs.util.DateUtils;
...
import com.cn.coachs.util.customgallery.utils.MediaScanner;
...
abstract class PhotoBaseActivity extends Activity {
    ...
    protected static Map<String, BeanPhotoInfo> mSelectPhotoMap = new HashMap<String, BeanPhotoInfo>();
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == GalleryHelper.TAKE_REQUEST_CODE) {
            if (resultCode == RESULT_OK && mTakePhotoUri != null) {
                ...
                final BeanPhotoInfo info = new BeanPhotoInfo();
     ...
     * 执行裁剪
     */
    protected void toPhotoCrop(BeanPhotoInfo info) {
    ...
    protected void resultMuti(ArrayList<BeanPhotoInfo> resultList) {
    ...
    protected void resultSingle(BeanPhotoInfo photoInfo) {
        Intent intent = getIntent();
        if (intent == null) {
            intent = new Intent();
        }
    ...
    protected abstract void takeResult(BeanPhotoInfo info);
