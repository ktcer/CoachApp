...
import com.cn.coachs.ui.chat.common.utils.DateUtil;
...
import com.cn.coachs.ui.chat.ui.group.DemoGroupNotice;
import com.cn.coachs.ui.chat.ui.group.GroupNoticeHelper;
...
public class IMChattingHelper implements OnChatReceiveListener, ECChatManager.OnDownloadMessageListener {
    ...
    public static long sendECMessage(ECMessage msg) {
        String to = msg.getTo();
        msg.setTo(Constant.AIHU_APPID + "#" + msg.getTo());
    ...
    public static long sendImageMessage(ImgInfo imgInfo, ECMessage message) {
        ...
        message.setTo(Constant.AIHU_APPID + "#" + message.getTo());
        ECChatManager manager = getInstance().mChatManager;
        if (manager != null) {
            // 调用接口发送IM消息
...
//                    clientUser.setSex(p.getSex());
    ...
    public void OnReceivedMessage(ECMessage msg) {
        ...
        msg.setSessionId(msg.getSessionId().replace(Constant.AIHU_APPID + "#", ""));
        msg.setTo(msg.getTo().replace(Constant.AIHU_APPID + "#", ""));
    ...
    private static void showNotification(ECMessage msg) {
        if (checkNeedNotification(msg.getSessionId())) {
            ...
            ECContacts contact = null;//= ContactSqlManager.getContact(msg.getForm());
            ...
            if (contact.getContactid().equals("921")) {
                contact.setNickname("管理员");
            } else {
                List<ECContacts> contactList = ContactsCache.getInstance().getContacts();
                for (ECContacts ecc : contactList) {
                    String SessionId = contact.getContactid();
                    String ListSessionId = ecc.getContactid();
                    if (SessionId.equals(ListSessionId)) {
                        contact.setNickname(ecc.getNickname());
                    }
                }
            }
    ...
    public void onReceiveOfflineMessageCompletion() {
        ...
        lastECMessage.setSessionId(lastECMessage.getSessionId().replace(Constant.AIHU_APPID + "#", ""));
        lastECMessage.setTo(lastECMessage.getTo().replace(Constant.AIHU_APPID + "#", ""));
        try {
