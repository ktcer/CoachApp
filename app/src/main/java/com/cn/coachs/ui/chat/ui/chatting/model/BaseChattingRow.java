            ...
            baseHolder.getChattingAvatar().setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    /*if(CCPAppManager.getClientUser().getUserId().equals(detail.getForm())) {
                        context.startActivity(new Intent(context, ActivityMsgSettings.class));
                        return ;
                    }*/
                    ECContacts contact = ContactSqlManager.getContact(detail.getForm());
...
//                    context.startActivity(intent);
                    ...
                    Intent intent = new Intent(context, ContactDetailActivity.class);
                    if ((userId + "1").equals(detail.getForm())) {
                        //传递本机用户的数据
                        //自己的头像不能点击！！！！
...
//                        intent.putExtra(ContactDetailActivity.DISPLAY_NAME, UtilsSharedData.getValueByKey(Constant.USER_NAME));
                        ...
                        intent.putExtra(Constant.FANS_NAME, ((ChattingActivity) context).getmUsername());
                        intent.putExtra(Constant.fANS_IMAGEURL, ((ChattingActivity) context).getmUserImgUrl());
                        intent.putExtra(Constant.FANS_GENDER, ((ChattingActivity) context).getmUserGender());
                        intent.putExtra(Constant.FANS_AGE, ((ChattingActivity) context).getmUserAge());
                        intent.putExtra(Constant.FANS_HEIGHT, ((ChattingActivity) context).getmUserHeight());
                        intent.putExtra(Constant.FANS_WEIGHT, ((ChattingActivity) context).getmUserWeight());
                        context.startActivity(intent);
                    }
...
//                }
                ...
                ImageLoader.getInstance().displayImage(AbsParam.getBaseUrl() + imgUrl,
                        ...
                        AppMain.initImageOptions(R.drawable.icon_touxiang_persion_gray, false));
