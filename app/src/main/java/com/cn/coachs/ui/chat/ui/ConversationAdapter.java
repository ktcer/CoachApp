...
//            }
            ...
            if (conversation.getSessionId().toUpperCase().startsWith("G")) {
                ...
                if (conversation.getSessionId().equals(GroupNoticeSqlManager.CONTACT_ID)) {
                    mViewHolder.user_avatar.setImageResource(R.drawable.default_user_icon);
                } else {
                    ECContacts contact = ContactSqlManager.getContact(conversation.getSessionId());
                    if (tempECContacts != null) {
                        ImageLoader.getInstance().displayImage(AbsParam.getBaseUrl() + tempECContacts.getImgUrl(), mViewHolder.user_avatar, AppMain.initImageOptions(R.drawable.icon_touxiang_persion_gray, false));
                    } else {
